...
void loop() {
  ...
  if (packetSize) {
    ...
    rightVel = rightWheelVelocity(linearVelocity, angularVelocity);
    leftVel = leftWheelVelocity(linearVelocity, angularVelocity);
...
void motorControl(float rightVelocity, float leftVelocity){
  right_duty = (255*abs(rightVelocity)/500);
  left_duty = (255*abs(leftVelocity)/500);
  ...
  if (right_duty>left_duty){
    analogWrite(En1, map(rightVelocity, 0,255,50,91));
    analogWrite(En2, map(leftVelocity, 0,255,0,50));
    digitalWrite(In1, HIGH);
    digitalWrite(In2, LOW);
    digitalWrite(In3, LOW);
    digitalWrite(In4, HIGH);
  }else if (right_duty<left_duty){
    analogWrite(En1, map(rightVelocity, 0,255,0,50));
    analogWrite(En2, map(leftVelocity, 0,255,50,91));
    digitalWrite(In1, LOW);
    digitalWrite(In2, HIGH);
    digitalWrite(In3, HIGH);
    digitalWrite(In4, LOW);
  }else if(right_duty==left_duty){
    analogWrite(En1, map(rightVelocity, 0,255,50,78));
    analogWrite(En2, map(leftVelocity, 0,255,50,78));
    if(rightVelocity>=0){
    digitalWrite(In1, HIGH);
    digitalWrite(In2, LOW);
