...
def arucoPositionTarget(img, marker, arucoId, X_target, Y_target, enemyCxy): #enemy centroid
    ...
    if (c_distance>50):
        if(arucoId==50 and c_distance>50 and not coFlagx):
            control_movement_x(r_distance,l_distance,c_distance, 0, arucoId)
        elif(arucoId==70 and c_distance>50 and not coFlagy):
            control_movement_y(r_distance,l_distance,c_distance, 0, arucoId)
        elif(arucoId==90 and c_distance>50 and not coFlagz):
        ...
        if(arucoId==50 and c_distance<50):
            coFlagx=True
        elif(arucoId==70 and c_distance<50):
            coFlagy=True
        elif(arucoId==90 and c_distance<50):
            coFlagz=True
...
# Function to find nearest robot to a corner
# def find_nearest_robot(corners, robot_positions):
#     min_distance = float('inf')
#     nearest_robot = None
#     for corner in corners:
#         for robot_position in robot_positions:
#             dist = distance(corner, robot_position)
#             if dist < min_distance:
#                 min_distance = dist
#                 nearest_robot = robot_position
#                 nearest
#     return nearest_robot, corner
def find_nearest_corner(robot_position, corners):
    min_distance = float('inf')
    nearest_corner = None
    for corner in corners:
...
def main():
    global arucoIds,coFlagx,coFlagy,coFlagz
    udpSocket.init()
                ...
                if 130 in markerDict.keys():
                    ...
                    if len(robot_positions)==0:
                        for i in markerDict.keys():
                            if i !=130:
                                posx,posy = getPosition(img,markerDict,i)
                                robot_positions.append([posx, posy])
                    print("Robot position: ",robot_positions)            
                    corners = [p1, p2, p3]
                    print("Corners: ",corners)
                    robot1 = find_nearest_corner(robot_positions[0], corners)
                    # corners.remove(robot1)
                    print(robot1)
                    robot2 = find_nearest_corner(robot_positions[1], corners)
                    print(robot2)
                    robot3 = find_nearest_corner(robot_positions[2], corners)
                    print(robot3)
                    
                    print(f"Robot-1:{robot1},\n Robot-2: {robot2},\n Robot-3: {robot3}\n")
                    cv2.line(img, robot1, corners[0], (255, 0, 0), 1)#Blue
                    cv2.line(img, robot2, corners[1], (0, 255, 0), 1)#green
                    cv2.line(img, robot3, corners[2], (0, 0, 255), 1)#red
